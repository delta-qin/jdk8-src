...
Klass* SystemDictionary::resolve_or_null(Symbol* class_name, Handle class_loader, Handle protection_domain, TRAPS) {
  ...
  //if (class_name == NULL)
     //std::cout << "CLASS NAME NULL"<<std::endl;
  assert(!THREAD->is_Compiler_thread(),
         err_msg("can not load classes with compiler thread: class=%s, classloader=%s",
                 class_name->as_C_string(),
                 class_loader.is_null() ? "null" : class_loader->klass()->name()->as_C_string()));
  //std::cout << "sdsd" << std::endl;
  //std::cout<<"CLASS NAME  " << class_name->as_C_string()<<std::endl;
...
instanceKlassHandle SystemDictionary::load_instance_class(Symbol* class_name, Handle class_loader, TRAPS) {
  instanceKlassHandle nh = instanceKlassHandle(); // null Handle
  std::string ddd = std::string (class_name->as_C_string ());
  if (ddd.find ("dacapo") != std::string::npos)
  {
    //std::cout<<"throwing exception " << class_name->as_C_string() << std::endl;
    //abort ();
  }

  if (class_loader.is_null()) {
    //std::cout<<"system " << class_name->as_C_string()<< std::endl;
    ...
    //std::cout<< "custom classloader " << class_name->as_C_string()<< "   " << (class_loader == loader) << std::endl;
