...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
        ...
        private final void contendedLock() {
            ...
            for (int s;;) {
                ...
                // 线程在等待获得锁的状态时，阻塞
                    ...
                    // 不会死循环，设置塞状态成功的时候会阻塞，唤醒之后再次循环，没拿到再次等待阻塞
    ...
    static final int spread(int h) {
        // 0x7f ff   ff ff   HASH_BITS 的作用是变正数
    ...
    // 自旋 + CAS + 双重检测  == 线程安全
    //              if ((sc = sizeCtl) < 0)  if ((tab = table) == null || tab.length == 0) {
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            if ((sc = sizeCtl) < 0)
                ...
                // 通过 CAS 设置 SIZECTL 变量的值，保证当前只有一个线程在初始化，-1代表当前只有一个线程能初始化
                // 保证了数组的初始化的安全性
            else if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                try {
                    //很有可能执行到这里的时候，table已经不为空了
                    //CAS 成功之后，还会再次判断当前数组是否已经初始化完成，如果已经初始化完成，就不会再次初始化
    ...
    private final void addCount(long x, int check) {
        // CounterCell 数组，实现并发计数，和LongAdder很像
        CounterCell[] as; long b, s;
        if ((as = counterCells) != null ||
                // CAS 设置 BASECOUNT 不成功
                ...
                synchronized (f) {
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            // 和jdk8 的hashmap实现是一样的，先拷贝出来两个链表，最后在循环外给新的数组赋值
                            // 在新数组位置上放置拷贝的值
